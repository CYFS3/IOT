...
//字库存放在磁盘中的路径
u8*const GBK24_PATH=(u8*)"FONT/GBK24.FON";		//GBK24的存放位置
u8*const GBK16_PATH=(u8*)"FONT/GBK16.FON";		//GBK16的存放位置
u8*const GBK12_PATH=(u8*)"FONT/GBK12.FON";		//GBK12的存放位置
u8*const UNIGBK_PATH=(u8*)"FONT/UNIGBK.BIN";		//UNIGBK.BIN的存放位置
...
{	
	...
	//先查找文件是否正常 
	strcpy((char*)pname,(char*)"0:");	//copy src内容到pname
	...
	{  
		...
		for(i=0;i<FONTSECSIZE;i++)	//先擦除字库区域,提高写入速度
		{
			...
			SPI_Flash_Read((u8*)buf,((FONTINFOADDR/512)+i)*512,512);//读出整个扇区的内容
			for(j=0;j<512;j++)//校验数据
			{
				if(buf[j]!=0XFFFFFFFF)break;//需要擦除  	  
			}
			if(j!=512)SPI_Flash_Erase_Sector((FONTINFOADDR/512)+i);	//需要擦除的扇区
		}
		free(buf);
