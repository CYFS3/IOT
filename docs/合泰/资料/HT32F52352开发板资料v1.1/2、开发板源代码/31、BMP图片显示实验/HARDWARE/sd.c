...
{
 	SPI1_SetSpeed(SPI_GUADTIME_16_SCK);//设置到低速模式	
}
//SD卡正常工作的时候,可以高速了
void SD_SPI_SpeedHigh(void)
{
 	SPI1_SetSpeed(SPI_GUADTIME_2_SCK);//设置到高速模式	
}
//SPI硬件层初始化
void SD_SPI_Init(void)
{
  //设置硬件上与SD卡相关联的控制引脚输出
	...
	AFIO_GPxConfig(GPIO_PD, GPIO_PIN_0, AFIO_FUN_GPIO);

                                                
	GPIO_DirectionConfig(HT_GPIOD, GPIO_PIN_0, GPIO_DIR_OUT);
	GPIO_SetOutBits(HT_GPIOD, GPIO_PIN_0);
