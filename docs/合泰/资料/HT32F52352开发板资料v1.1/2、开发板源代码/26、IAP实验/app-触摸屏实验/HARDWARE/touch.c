...
float TouchFacX=0;//保存x校正因素  
float TouchFacY=0;//保存y校正因素 
short TouchXoff=0;//保存x偏移量
short TouchYoff=0;//保存y偏移量

#define TDIN GPIO_ReadInBit(HT_GPIOA,GPIO_PIN_6)
#define PEN  GPIO_ReadInBit(HT_GPIOB,GPIO_PIN_8)


void DOUT(u8 a)
{
	if(a) GPIO_SetOutBits(HT_GPIOA,GPIO_PIN_5);
	else GPIO_ClearOutBits(HT_GPIOA,GPIO_PIN_5);
}

void TCLK(u8 a)
{
	if(a) GPIO_SetOutBits(HT_GPIOA,GPIO_PIN_4);
	else GPIO_ClearOutBits(HT_GPIOA,GPIO_PIN_4);
}

void TCS(u8 a)
{
	if(a) GPIO_SetOutBits(HT_GPIOA,GPIO_PIN_7);
	else GPIO_ClearOutBits(HT_GPIOA,GPIO_PIN_7);
}
...
{			   
	if(PEN==0)//有按键按下
	{
		if(tp)TP_Read_XY2(&TouchX,&TouchY);//读取物理坐标
		else if(TP_Read_XY2(&TouchX,&TouchY))//读取屏幕坐标
		{
			if(TouchX>10 && TouchX<4050 && TouchY>10 && TouchY<4050)
			{
				TouchX=TouchFacX*TouchX+TouchXoff;//将结果转换为屏幕坐标
				TouchY=TouchFacY*TouchY+TouchYoff;  
			}
				
	 	} 
		if((TouchSta&TP_PRES_DOWN)==0)//之前没有被按下
		{		 
			TouchSta=TP_PRES_DOWN|TP_CATH_PRES;//按键按下  
			TouchXBak=TouchX;//记录第一次按下时的坐标
			TouchYBak=TouchY;  	   			 
		}			   
	}else
	{
		if(TouchSta&TP_PRES_DOWN)//之前是被按下的
		{
			TouchSta&=~(1<<7);//标记按键松开	
		}else//之前就没有被按下
		{
	...
	return TouchSta&TP_PRES_DOWN;//返回当前的触屏状态
}	  
//////////////////////////////////////////////////////////////////////////	 
...
{  
	s32 temp;			 
	//保存校正结果!		   							  
	temp=TouchFacX*100000000;//保存x校正因素      
    AT24CXX_WriteLenByte(SAVE_ADDR_BASE,temp,4);   
	temp=TouchFacY*100000000;//保存y校正因素    
    AT24CXX_WriteLenByte(SAVE_ADDR_BASE+4,temp,4);
	//保存x偏移量
    AT24CXX_WriteLenByte(SAVE_ADDR_BASE+8,TouchXoff,2);		    
	//保存y偏移量
	AT24CXX_WriteLenByte(SAVE_ADDR_BASE+10,TouchYoff,2);	
	//保存触屏类型
...
{		  
	...
	{    												 
		...
		TouchFacX=(float)tempfac/100000000;//得到x校准参数
		tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+4,4);			          
		TouchFacY=(float)tempfac/100000000;//得到y校准参数
	    //得到x偏移量
		TouchXoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+8,2);			   	  
 	    //得到y偏移量
		TouchYoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+10,2);				 	  
...
{								 
	...
	TouchSta=0;//消除触发信号 
	TouchFacX=0;//xfac用来标记是否校准过,所以校准之前必须清掉!以免错误	 
	while(1)//如果连续10秒钟没有按下,则自动退出
	{
READJ:
		TP_Scan(1);//扫描物理坐标
		if((TouchSta&0xc0)==TP_CATH_PRES)//按键按下了一次(此时按键松开了.)
		{	
			outtime=0;		
			TouchSta&=~(1<<6);	//标记按键已经被处理过了. 
			pos_temp[cnt][0]=TouchX;
			pos_temp[cnt][1]=TouchY;
			cnt++;	  
			switch(cnt)
			{			   
				case 1:						 
				...
				case 4:	 //全部四个点已经得到
					...
					//正确了
					//计算结果
					TouchFacX=(float)(lcddev.width-40)/(pos_temp[1][0]-pos_temp[0][0]);//得到xfac		 
					TouchXoff=(lcddev.width-TouchFacX*(pos_temp[1][0]+pos_temp[0][0]))/2;//得到xoff
 					TouchFacY=(float)(lcddev.height-40)/(pos_temp[2][1]-pos_temp[0][1]);//得到yfac
					TouchYoff=(lcddev.height-TouchFacY*(pos_temp[2][1]+pos_temp[0][1]))/2;//得到yoff  
					if(abs(TouchFacX)>2||abs(TouchFacY)>2)//触屏和预设的相反了.
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20,WHITE);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED);								//画点1
...
{		
	...
	//配置端口功能为GPIO
	AFIO_GPxConfig(GPIO_PA, GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7, AFIO_FUN_GPIO);
    AFIO_GPxConfig(GPIO_PB, GPIO_PIN_8, AFIO_FUN_GPIO);
	
	GPIO_DirectionConfig(HT_GPIOA, GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_7, GPIO_DIR_OUT);
	...
	TP_Read_XY(&TouchX,&TouchY);//第一次读取初始化	
