...
#define MPU_SDA_IN()  {GPIO_DirectionConfig(HT_GPIOA, GPIO_PIN_1, GPIO_DIR_IN);GPIO_InputConfig(HT_GPIOA, GPIO_PIN_1,ENABLE);}
#define MPU_SDA_OUT() {GPIO_DirectionConfig(HT_GPIOA, GPIO_PIN_1, GPIO_DIR_OUT);GPIO_InputConfig(HT_GPIOA, GPIO_PIN_1,DISABLE);}
  
#define MPU_READ_SDA   GPIO_ReadInBit(HT_GPIOA,GPIO_PIN_1)  


void MPU_IIC_SCL(u8 x)
{
	if(x)
...
{                        
    ...
    for(t=0;t<8;t++)
    {              
        if(txd&0x80)MPU_IIC_SDA(1);else MPU_IIC_SDA(0);
        txd<<=1; 	  
		MPU_IIC_Delay(); 
		    MPU_IIC_SCL(1);
		    MPU_IIC_Delay(); 
